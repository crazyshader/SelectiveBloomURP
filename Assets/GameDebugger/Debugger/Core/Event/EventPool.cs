...
namespace GameFramework.Event
{
	...
	internal sealed partial class EventPool<T> where T : GameEventArgs
	{
		...
		public void Unsubscribe(int id, EventHandler<T> handler)
		{
			...
			if (!m_EventHandlers.TryGetValue(id, out handlers))
			{
				throw new Exception(Utility.Text.Format("Event '{0}' not exists any handler.", id.ToString()));
			}
		...
		private void HandleEvent(object sender, T e)
		{
			...
			{
				throw new Exception(Utility.Text.Format("Event '{0}' not allow no handler.", eventId.ToString()));
			}
		}
	}
}
